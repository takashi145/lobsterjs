...
**水平結合** — `\|` はマージマーカーである。2つのセル間の `|` セパレーターの直前に `\` がある場合、左セルが2列分のcolspanを持つ。コンテンツを左セルに書き、右セルを空にする:

```
| A | B \|   |
```

> **制約:** `\|` の両側にコンテンツがある場合（例: `| A \| B |`）、右側のコンテンツ（B）は無視される。左側のコンテンツ（A）のみが保持され、colspan=2が付与される。結合を明示する場合は `| A \|   |` のように書く。
...
サイレントテーブルにはヘッダー行と整列行が最低限必要。ワープ参照はヘッダー行（パターンA）またはデータ行（パターンB）のどちらにも書ける。

**パターンA — ヘッダー行にワープ参照**（コンテンツは `<th>` に出力される）:
...
**パターンB — データ行にワープ参照**（コンテンツは `<td>` に出力される。レイアウト用途に推奨）:
